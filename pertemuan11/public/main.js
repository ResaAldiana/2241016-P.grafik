...
function drawBlades(rotationAngle, centerX, centerY) {
  ...
  gl.uniform4fv(colorLocation, [1.0, 1.0, 0.0, 1.0]); // Mengatur warna kuning
  ...
  for (var i = 0; i < numBlades; i++) {
    ...
    var p1 = [
      Math.cos(angle) * 0.1 + center[0],
      Math.sin(angle) * 0.1 + center[1],
    ];
    var p2 = [
...
function updateRotations() {
  ...
  verticalPosition += 0.01; // Atur kecepatan gerakan vertikal di sini
  if (verticalPosition > 1.0) {
    verticalPosition = -1.0; // Reset posisi ke bawah setelah mencapai atas
  }
}
...
function animateBlades() {
  ...
  // Menggambar satu objek baling-baling di tengah canvas dengan posisi vertikal yang berubah
  drawBlades(rotationAngleClockwise, 0, verticalPosition);
